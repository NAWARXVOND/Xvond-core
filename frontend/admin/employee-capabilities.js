function employeeForm(d={},edit=false){
  return `<p style="margin-top:0">Configure only the employee identity and conversation behavior here. Business facts belong in Knowledge; real business operations belong in Actions.</p>
  ...
  <div class="form-group"><label>Business Type</label><select id="simple-business-type">${businessTypeOptions(d.business_type||"")}</select><small>This helps Xvond suggest the right starting template; it never limits what the employee can do.</small></div>
  ...
  <div style="padding:12px;border:1px solid #e5e7eb;border-radius:10px;margin:12px 0"><strong>Single source of truth</strong><div class="meta" style="margin-top:6px">Knowledge: everything the employee knows about the business.</div><div class="meta">Actions: everything the employee can actually do and where it executes.</div><div class="meta">Channels: Website Chat, WhatsApp and future communication surfaces only.</div></div>
...
async function createSimpleAIEmployee(){try{const payload=collectEmployee();const created=await api(`/admin/ai-employees/companies/${simpleCompanyId}`,{method:"POST",body:JSON.stringify({channel:"whatsapp",name:payload.name,business_name:payload.business_name,business_type:payload.business_type,reply_language:payload.reply_language,instructions:payload.instructions})});const agentId=created?.employee?.id;if(agentId){await api(`/admin/ai-employee-profile/companies/${simpleCompanyId}/${agentId}`,{method:"PUT",body:JSON.stringify(payload)})}closeModal();await openSimpleCompany(simpleCompanyId)}catch(e){alert(e.message)}}
