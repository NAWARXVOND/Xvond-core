...
function xaPlanCards(plans){
  if(!plans.length)return wsEmpty('No service packages yet','Create the first package, then assign it to a company service.');
  return `<div class="integration-grid">${plans.map(plan=>`<div class="integration-card"><div class="integration-card-head"><div><h4>${f(plan.name)}</h4><div class="meta">${f(XVOND_BILLING_SERVICE_LABELS?.[plan.service_code]||plan.service_code)} · ${f(plan.tier)}</div></div>${wsPill(`${plan.currency} ${xaNumber(plan.monthly_price)}`,'neutral')}</div><div class="meta">${billingPlanLimitsSummary(plan)}</div><div class="workspace-inline-actions"><button class="table-button" onclick="openWorkspaceEditServicePlan(${plan.id})">Edit Package</button></div></div>`).join('')}</div>`;
}
...
openWorkspaceServiceForm=function(serviceCode=null){
  ...
  if(!plans.length){openWorkspaceCreateServicePlan();return;}
