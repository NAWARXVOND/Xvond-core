...
from backend.app.modules.channels.whatsapp_models import WhatsAppSession, WhatsAppInboundMessage
...
def database(monkeypatch):
    ...
    with factory() as db:
        db.add_all([Company(id=1, name="Saved Business", active=True), Company(id=2, name="Other Tenant", active=True)])
        db.flush()
        db.add_all([AIAgent(id=1, company_id=1, name="Employee", system_prompt="base", provider="mock", model="mock"), AIAgent(id=2, company_id=2, name="Other", system_prompt="base", provider="mock", model="mock")])
        db.add_all([User(id=1, company_id=1, full_name="Owner", email="owner@example.test", password_hash="unused", role="owner", active=True), User(id=2, company_id=1, full_name="Staff", email="staff@example.test", password_hash="unused", role="employee", active=True)])
        db.flush()
        db.add_all([AgentChannel(id=1, company_id=1, agent_id=1, channel_type="whatsapp", enabled=True, config={"phone_number_id":"phone-1", "app_secret":"test-signing-secret", "access_token":"test-send-secret", "graph_api_version":"v26.0"}), AgentChannel(id=2, company_id=2, agent_id=2, channel_type="whatsapp", config={"phone_number_id":"phone-2"})])
        db.commit()
    monkeypatch.setattr(webhook, "find_channel_by_phone_number_id", lambda db, phone: db.get(AgentChannel, 1) if phone == "phone-1" else None)
    monkeypatch.setattr(webhook.agent_runtime, "get_agent", lambda db, company, agent: db.get(AIAgent, agent))
    def chat(db, message, conversation_id, **kwargs):
        db.add_all([AIMessage(conversation_id=conversation_id, role="user", content=message), AIMessage(conversation_id=conversation_id, role="assistant", content="Verified reply")])
        return {"conversation_id": conversation_id, "response": {"content": "Verified reply"}}
    monkeypatch.setattr(webhook.agent_runtime, "chat", chat)
    monkeypatch.setattr(webhook.whatsapp_sender, "send_text", lambda **kwargs: {"success": True, "status_code":200, "response":{"messages":[{"id":"sent-1"}]}})
    yield factory
    engine.dispose()
...
def test_delivery_failure_rolls_back_claim_and_messages_then_retries(database, monkeypatch):
    monkeypatch.setattr(webhook.whatsapp_sender, "send_text", lambda **kwargs: {"success":False})
    with pytest.raises(RuntimeError, match="delivery failed"):
        process()
    with database() as db:
        assert db.query(WhatsAppInboundMessage).count() == 0
        assert db.query(AIMessage).count() == 0
    monkeypatch.setattr(webhook.whatsapp_sender, "send_text", lambda **kwargs: {"success":True})
    process()
    assert process()["processed"][0]["status"] == "duplicate"


def test_portal_claim_owner_send_failure_and_explicit_return(database, monkeypatch):
    conversation_id = process()["processed"][0]["conversation_id"]
    inbox.take_over_conversation(conversation_id, user())
    ...
    monkeypatch.setattr(webhook.whatsapp_sender, "send_text", lambda **kwargs: {"success":False})
    with pytest.raises(HTTPException) as exc:
        inbox.send_human_reply(conversation_id, inbox.HumanReply(message="Manual"), user())
    assert exc.value.status_code == 502
    with database() as db:
        assert db.query(AIMessage).filter_by(role="human").count() == 0
    sent = []
    monkeypatch.setattr(webhook.whatsapp_sender, "send_text", lambda **kwargs: sent.append(kwargs) or {"success":True})
    inbox.send_human_reply(conversation_id, inbox.HumanReply(message="Manual"), user())
    assert sent[0]["to"] == "contact-1"
    assert sent[0]["config"]["phone_number_id"] == "phone-1"
    inbox.return_conversation_to_ai(conversation_id, user())
    with database() as db:
...
def test_default_inbox_excludes_test_unknown_and_other_tenants(database):
    ...
    with database() as db:
        db.add_all([AIConversation(company_id=1, agent_id=1, channel_type="portal_test", title="Test"), AIConversation(company_id=1, agent_id=1, title="Unknown"), AIConversation(company_id=2, agent_id=2, channel_type="whatsapp", title="Private")])
        db.commit()
    ...
    with database() as db:
        for channel_id, contact in [(2,"contact-1"),(1,"different-contact")]:
            with pytest.raises(ValueError):
                bind_conversation_source(db, conversation_id=conversation_id, company_id=1, agent_id=1, channel_type="whatsapp", channel_id=channel_id, external_contact_id=contact)


def test_identity_edit_preserves_services_and_syncs_exact_saved_facts(database):
    with database() as db:
        db.add(CompanyProfile(company_id=1, services=[{"name":"Consultation", "price":"25 OMR"}], policies=["Appointment required"]))
        db.commit()
    result = profile_api.update_company_profile(1, profile_api.CompanyProfileUpdate(company_name="Renamed Business"), user())
    ...
    result = profile_api.update_company_profile(1, profile_api.CompanyProfileUpdate(company_name="Renamed Business", services=[]), user())
...
def test_embedding_timeout_keeps_saved_knowledge_and_does_not_log_secrets(database, monkeypatch, caplog):
    ...
    profile_api.update_company_profile(1, profile_api.CompanyProfileUpdate(company_name="Business", services=["Consultation"]), user())
    with database() as db:
        knowledge_service._live_backfill_after.clear()
        context = knowledge_service.get_agent_context(db, 1, 1, "What services are available?")
