#!/usr/bin/env python3
"""Install Xvond Core routes into the existing xvond.com nginx vhost safely.
...
def _contains_domain_server(text: str) -> bool:
    return bool(re.search(r"\bserver_name\s+[^;]*\bxvond\.com\b[^;]*;", text))
...
def _discover_target() -> Path:
    candidates = [
        ...
        if _contains_domain_server(text)
        and path.exists()
        and path.is_file()
...
def _server_bounds(lines: list[str]) -> tuple[int, int]:
    domain_line = None
    for index, line in enumerate(lines):
        if re.search(r"\bserver_name\s+[^;]*\bxvond\.com\b[^;]*;", _strip_comment(line)):
            domain_line = index
            break
...
def _install_include(target: Path) -> tuple[Path, bool]:
    original = target.read_text(encoding="utf-8")
    if str(INCLUDE_PATH) in original:
        return target, False

    lines = original.splitlines(keepends=True)
    start, end = _server_bounds(lines)
    insertion = end
...
def main() -> int:
    ...
    _, changed = _install_include(target)
