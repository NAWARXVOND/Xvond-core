...
def get_config(agent_id:int,current_admin:User=Depends(require_xvond_admin)):
    ...
    try:
        ...
        cfg=reveal_config(ch.config);blockers=_ready(db,ch)
        safe={k:v for k,v in cfg.items() if k!="widget_key"}
