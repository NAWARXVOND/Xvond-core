...
router = APIRouter(prefix="/admin/voice", tags=["Xvond Admin - Voice"])
...
def _public_base_url() -> str:
    value = str(os.getenv("PUBLIC_BASE_URL") or "").strip().rstrip("/")
    if not value:
        raise HTTPException(
            status_code=503,
            detail="PUBLIC_BASE_URL is not configured on the Xvond server",
        )
    if not value.lower().startswith("https://"):
        raise HTTPException(
            status_code=503,
            detail="PUBLIC_BASE_URL must use HTTPS for production voice calls",
        )
    return value
...
def provision_vapi_voice_channel(
    ...
    try:
        ...
        if not phone_number_id:
            raise HTTPException(status_code=400, detail="A real Vapi phone number is required")

        phone = get_phone_number(phone_number_id)
        phone_number = str(phone.get("number") or phone.get("phoneNumber") or "").strip()
        if not phone_number:
            raise HTTPException(
                status_code=502,
