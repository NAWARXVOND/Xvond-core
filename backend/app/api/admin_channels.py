...
def _activation_blockers(db, channel: AgentChannel) -> list[str]:
    ...
    if agent is None or not agent.enabled:
        ...
        blockers.append("A real AI provider is required")
    if not _channel_configured(channel):
        blockers.append("WhatsApp credentials are incomplete")
    ...
    if not any(len((doc.content or "").strip()) >= 20 for doc in useful_knowledge):
        blockers.append("Add real business knowledge before activating WhatsApp")
    return blockers
...
def create_channel(agent_id: int, data: ChannelCreate, current_admin: User = Depends(require_xvond_admin)):
    ...
    try:
        ...
        existing = db.query(AgentChannel).filter(AgentChannel.agent_id == agent.id, AgentChannel.channel_type == channel_type).first()
        ...
        channel = AgentChannel(company_id=agent.company_id, agent_id=agent.id, channel_type=channel_type, config=data.config, enabled=False)
        db.add(channel)
...
def list_company_channels(company_id: int, current_admin: User = Depends(require_xvond_admin)):
    db = SessionLocal()
    try:
        items = db.query(AgentChannel).filter(AgentChannel.company_id == company_id).order_by(AgentChannel.id.asc()).all()
...
def update_channel(channel_id: int, data: ChannelUpdate, current_admin: User = Depends(require_xvond_admin)):
    ...
    try:
        ...
        if data.enabled is True and channel.enabled is False:
            ...
            blockers = _activation_blockers(db, channel)
            if blockers:
                raise HTTPException(409, detail={"message": "WhatsApp is not ready for activation", "blockers": blockers})
            _ensure_channels_module(db, channel.company_id)
            channel.enabled = True
        elif data.enabled is False:
            channel.enabled = False
        db.commit(); db.refresh(channel)
        result = serialize_channel(channel); result["status"] = "updated"; return result
...
def configure_whatsapp(channel_id: int, data: WhatsAppConfigUpdate, current_admin: User = Depends(require_xvond_admin)):
    ...
    try:
        ...
        # Configuration and activation are separate on purpose. This prevents a
        # partially-trained employee from going live when credentials are saved.
        channel.enabled = False
        _ensure_channels_module(db, channel.company_id)
        db.commit(); db.refresh(channel)
        blockers = _activation_blockers(db, channel)
