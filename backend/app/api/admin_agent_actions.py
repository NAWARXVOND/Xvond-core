...
VALID_REQUEST_STATUSES={"new","pending_human","in_progress","confirmed","processing","completed","cancelled"}
...
def _assignment(db,agent_id,name):return db.query(AgentToolAssignment).filter(AgentToolAssignment.agent_id==agent_id,AgentToolAssignment.tool_name==name).first()
def _set(db,agent_id,name,enabled,config):
    row=_assignment(db,agent_id,name)
    if row:row.enabled=enabled;row.config=config
    else:db.add(AgentToolAssignment(agent_id=agent_id,tool_name=name,enabled=enabled,config=config))
def _fields(values):
    out=[]
...
def get_actions(agent_id:int,current_admin:User=Depends(require_xvond_admin)):
    ...
    try:
        ...
        request=_assignment(db,agent_id,"action_request");cfg=(request.config or {}) if request else {};actions=cfg.get("actions") or {};booking=_assignment(db,agent_id,"booking");order=_assignment(db,agent_id,"order");lead=_assignment(db,agent_id,"lead")
        ...
        booking_cfg=(booking.config or {}) if booking else {};order_cfg=(order.config or {}) if order else {}
        return {"booking_mode":bm,"booking_fields":booking_cfg.get("required_fields") or actions.get("booking",{}).get("required_fields") or ["customer_name","phone","service","date","time"],"order_mode":om,"order_fields":order_cfg.get("required_fields") or actions.get("order",{}).get("required_fields") or ["customer_name","phone","request"],"lead_enabled":bool(lead and lead.enabled),"lead_fields":((lead.config or {}).get("required_fields") if lead else None) or ["name","phone"]}
    finally:db.close()
...
def update_actions(agent_id:int,data:ActionSettings,current_admin:User=Depends(require_xvond_admin)):
    ...
    try:
        ...
        bf=_fields(data.booking_fields);of=_fields(data.order_fields);lf=_fields(data.lead_fields)
        _set(db,agent_id,"booking",data.booking_mode=="xvond_internal",{"approval_required":False,"mode":"xvond_internal","required_fields":bf})
        _set(db,agent_id,"order",data.order_mode=="xvond_internal",{"approval_required":False,"mode":"xvond_internal","required_fields":of})
        _set(db,agent_id,"lead",data.lead_enabled,{"approval_required":False,"required_fields":lf})
        _set(db,agent_id,"human_handoff",True,{"approval_required":False})
        actions={}
        if data.booking_mode=="human_handoff":actions["booking"]={"enabled":True,"required_fields":bf,"department":"bookings"}
        if data.order_mode=="human_handoff":actions["order"]={"enabled":True,"required_fields":of,"department":"orders"}
