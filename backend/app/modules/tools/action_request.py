...
class ActionRequestTool(AgentTool):
    ...
    def execute(self, arguments, context):
        ...
        allowed_type = str(config.get("action_type") or "").strip()
        action_type = str(arguments.get("action_type") or "").strip()
        if not allowed_type or action_type != allowed_type:
        ...
        required_fields = [str(x).strip() for x in (config.get("required_fields") or []) if str(x).strip()]
        ...
        handoff = HumanHandoff(
            ...
            department=str(config.get("department") or "customer_service"),
        )
        db.add(handoff)
        db.flush()
        session = None
