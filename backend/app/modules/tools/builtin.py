...
class BookingTool(AgentTool):
 name="booking";description="Check availability and create, reschedule or cancel a real booking in Xvond's internal booking store. Never confirm a booking unless this tool returns success."
 ...
 def execute(self,arguments,context):
  ...
  if action in ("check_availability","create"):
   ...
   conflict=db.query(Booking).filter(Booking.company_id==cid,Booking.booking_date==str(arguments["date"]),Booking.booking_time==str(arguments["time"]),Booking.status=="confirmed").first()
   if action=="check_availability":return ToolResult(success=True,data={"action":"availability_checked","available":conflict is None,"date":arguments["date"],"time":arguments["time"],"service":arguments["service"]})
   ...
   if conflict:return ToolResult(success=False,error="Requested booking time is not available",data={"available":False,"date":arguments["date"],"time":arguments["time"]})
   booking=Booking(company_id=cid,agent_id=aid,customer_name=arguments["customer_name"],phone=arguments["phone"],service=arguments["service"],booking_date=arguments["date"],booking_time=arguments["time"],status="confirmed");db.add(booking);db.flush();return ToolResult(success=True,data={"action":"booking_created","booking_id":booking.id,"status":"confirmed","date":booking.booking_date,"time":booking.booking_time,"service":booking.service})
  ...
  if action=="reschedule":
   date=arguments.get("date") or booking.booking_date;time=arguments.get("time") or booking.booking_time
   conflict=db.query(Booking).filter(Booking.company_id==cid,Booking.booking_date==str(date),Booking.booking_time==str(time),Booking.status=="confirmed",Booking.id!=booking.id).first()
   if conflict:return ToolResult(success=False,error="Requested booking time is not available",data={"available":False,"date":date,"time":time})
   booking.booking_date=str(date);booking.booking_time=str(time);booking.status="confirmed";db.flush();return ToolResult(success=True,data={"action":"booking_rescheduled","booking_id":booking.id,"date":booking.booking_date,"time":booking.booking_time})
  return ToolResult(success=False,error="Invalid booking action")
class OrderTool(AgentTool):
 name="order";description="Create and save a real customer order. Never confirm an order unless this tool returns success.";input_schema={"type":"object","properties":{"customer_name":{"type":"string"},"phone":{"type":"string"},"items":{"type":"array","items":{"type":"object"},"minItems":1},"delivery_address":{"type":"string"},"notes":{"type":"string"}},"required":["items"],"additionalProperties":False}
 def execute(self,arguments,context):
  db=context["db"];items=arguments.get("items",[])
...
class WebhookTool(AgentTool):
 ...
 def execute(self,arguments,context):
  ...
  try:
   validate_public_http_url(url);result=safe_http_request(url=url,method=config.get("method","POST"),headers={"Content-Type":"application/json",**(config.get("headers",{}) or {})},json_data=arguments.get("payload",arguments),timeout=config.get("timeout",15));status=int(result["status_code"])
...
class CustomAPITool(AgentTool):
 ...
 def execute(self,arguments,context):
  ...
  endpoint=str(arguments.get("endpoint","") or "").strip();lowered=endpoint.lower()
  if lowered.startswith("http://") or lowered.startswith("https://") or endpoint.startswith("//"):return ToolResult(success=False,error="API endpoint must be a relative path")
