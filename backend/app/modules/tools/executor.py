...
def tool_requires_approval(tool_name:str,config:dict|None)->bool:
 config=config or {}
 if "approval_required" in config:return bool(config["approval_required"])
...
class ToolExecutor:
 ...
 def get_agent_tools(self,db,agent_id:int)->list[dict]:
  ...
  for assignment in assignments:
   ...
   if tool is not None:result.append({"name":tool.name,"description":tool.description,"input_schema":tool.input_schema})
  return result
 ...
 def execute(self,db,company_id:int,agent_id:int,tool_name:str,arguments:dict,conversation_id:int|None=None,approval_granted:bool=False)->dict:
  ...
  config=reveal_config(assignment.config);approval_required=tool_requires_approval(tool_name,config)
