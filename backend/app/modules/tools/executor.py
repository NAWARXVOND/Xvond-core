...
def tool_requires_approval(tool_name: str, config: dict | None) -> bool:
    config = config or {}
    # External side-effect tools are always protected. Assignment config may make
    # additional tools stricter, but it may never weaken the sensitive-tool policy.
...
class ToolExecutor:
    ...
    def get_agent_tools(self, db, agent_id: int) -> list[dict]:
        ...
        for assignment in assignments:
            ...
            if tool is not None:
                result.append({"name": tool.name, "description": tool.description, "input_schema": tool.input_schema})
        return result
    ...
    def execute(self, db, company_id: int, agent_id: int, tool_name: str, arguments: dict, conversation_id: int | None = None, approval_granted: bool = False) -> dict:
        ...
        try:
            with db.begin_nested():
                result = tool.execute(arguments=arguments or {}, context=context)
                db.flush()
