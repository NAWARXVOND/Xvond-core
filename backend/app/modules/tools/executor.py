...
def _runtime_description(tool, config: dict) -> str:
    ...
    if tool.name == "action_request":
        ...
        for action_type, action in actions.items():
            if not isinstance(action,dict) or not action.get("enabled",True):continue
            fields=[str(x).strip() for x in (action.get("required_fields") or []) if str(x).strip()]
            rules.append(f"{action_type}: required customer details = {', '.join(fields) if fields else 'none'}")
        if rules:
            description += " CONFIGURED REQUESTS: " + " | ".join(rules) + ". Collect only missing details progressively in natural conversation. Remember details already supplied. Before submitting, briefly summarize the completed request and obtain customer confirmation. Do not call this tool until every configured required detail is present and the customer has confirmed."
    if tool.name == "lead":
...
class ToolExecutor:
    ...
    def get_agent_tools(self, db, agent_id: int) -> list[dict]:
        assignments = db.query(AgentToolAssignment).filter(AgentToolAssignment.agent_id == agent_id, AgentToolAssignment.enabled.is_(True)).order_by(AgentToolAssignment.id.asc()).all()
        result = []
        for assignment in assignments:
            tool = tool_registry.get(assignment.tool_name)
            if tool is not None:
                config=reveal_config(assignment.config) or {}
                result.append({"name": tool.name, "description": _runtime_description(tool,config), "input_schema": tool.input_schema})
        return result
    ...
    def execute(self, db, company_id: int, agent_id: int, tool_name: str, arguments: dict, conversation_id: int | None = None, approval_granted: bool = False) -> dict:
        ...
        if scope_error is not None:return {"success": False, "tool": tool_name, "data": None, "error": scope_error}
        require_company_module(db, company_id, "tools")
        assignment = db.query(AgentToolAssignment).filter(AgentToolAssignment.agent_id == agent_id, AgentToolAssignment.tool_name == tool_name, AgentToolAssignment.enabled.is_(True)).first()
        ...
        if approval_required and not approval_granted:
            request = ToolApprovalRequest(company_id=company_id, agent_id=agent_id, conversation_id=conversation_id, tool_name=tool_name, arguments=arguments or {}, status="pending")
        ...
        try:
            with db.begin_nested():
                result = tool.execute(arguments=arguments or {}, context=context);db.flush()
