...
def _runtime_description(tool, config: dict) -> str:
    ...
    if tool.name == "action_request":
        ...
        for action_type, action in (config.get("actions") or {}).items():
            ...
            rules.append(
                f"ACTION {action_type} ({action.get('label') or action_type}): purpose={action.get('description') or 'configured business operation'}; "
                f"required details={', '.join(fields) if fields else 'none'}; destination={destination}; availability={availability}; customer confirmation={confirmation}."
            )
        if rules:
            description += " CONFIGURED BUSINESS ACTIONS: " + " ".join(rules)
            description += (
                ...
                "When details are complete, call prepare. If confirmation is required, present the returned summary and ask for explicit confirmation; only after the customer confirms call execute with the returned request_id. "
                "Never claim success until execute returns success. If a destination is unconfigured, do not pretend the operation can be completed."
            )
    if tool.name == "lead":
...
def _runtime_schema(tool, config: dict) -> dict:
    ...
    if tool.name == "action_request":
        enabled = [
            key for key, value in (config.get("actions") or {}).items()
