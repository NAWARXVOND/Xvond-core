...
You are operating as an employee of this business, not as a general-information chatbot.
For any business-specific fact (prices, menu/items, services, products, opening hours, branches, addresses, policies, offers, availability, stock, delivery, booking rules, order status, contact details), use only COMPANY KNOWLEDGE or successful connected-tool results supplied in this request.
Never fill missing business facts from general knowledge, assumptions, memory, or plausible guesses.
If the requested business fact is not present in COMPANY KNOWLEDGE and no tool can retrieve it, clearly say that the information is not available to you and offer human assistance when appropriate.
Do not contradict COMPANY KNOWLEDGE. If conversation history conflicts with current COMPANY KNOWLEDGE, current COMPANY KNOWLEDGE wins.
Never claim that a booking, order, cancellation, reschedule, payment, or other action succeeded unless the corresponding tool returned success.
"""
...
class AgentRuntime:
    ...
    def build_history(self,db,conversation_id:int)->str:
        messages=db.query(AIMessage).filter(AIMessage.conversation_id==conversation_id).order_by(AIMessage.id.desc()).limit(40).all();messages.reverse()
        return "\n".join(f"{item.role}: {item.content}" for item in messages)[-12000:]
    MAX_TOOL_ROUNDS=6
    def chat(self,db,company_id:int,agent_id:int,message:str,conversation_id:int|None=None)->dict:
        ...
        self.assert_company_runtime_access(db,company_id);limits_service.check_token_limit(db,company_id);agent=self.get_agent(db,company_id,agent_id)
        selections=runtime_selections(db,company_id,agent.provider,agent.model);active_provider=selections[0].provider;active_model=selections[0].model
        conversation=self.get_or_create_conversation(db,company_id,agent.id,conversation_id,message);history=self.build_history(db,conversation.id)
        knowledge=""
        ...
        available_tools=tool_executor.get_agent_tools(db=db,agent_id=agent.id) if company_module_enabled(db,company_id,"tools") else []
        tool_definitions=[{"name":t["name"],"description":t["description"],"input_schema":t.get("input_schema",{"type":"object","properties":{}})} for t in available_tools]
        context_parts=[GROUNDING_POLICY]
        if knowledge:context_parts.append("COMPANY KNOWLEDGE (authoritative for this turn):\n"+knowledge)
        else:context_parts.append("COMPANY KNOWLEDGE (authoritative for this turn):\nNo relevant business knowledge was retrieved for this question. Do not invent business facts.")
        if history:context_parts.append("CONVERSATION HISTORY (context only; not authoritative for business facts):\n"+history)
        context_parts.append("CURRENT USER MESSAGE:\n"+message);runtime_message="\n\n".join(context_parts)
        user_message=AIMessage(conversation_id=conversation.id,role="user",content=message);db.add(user_message);db.flush()
        tool_outputs=None;continuation=None;executed_tools=[];total_input_tokens=0;total_output_tokens=0;total_tokens=0;total_provider_cost=Decimal("0");final_text="";started_at=perf_counter()
        for _round in range(self.MAX_TOOL_ROUNDS):
            try:
                try:result=ai_engine.generate(provider_name=active_provider,system_prompt=agent.system_prompt,user_message=runtime_message,model=active_model,tools=tool_definitions,tool_outputs=tool_outputs,continuation=continuation)
                    ...
                    fallback=selections[1];active_provider=fallback.provider;active_model=fallback.model
            ...
            total_input_tokens+=result.input_tokens;total_output_tokens+=result.output_tokens;total_tokens+=result.total_tokens
            calculated_cost=ai_cost_engine.calculate(db=db,provider_name=active_provider,model_name=active_model,input_tokens=result.input_tokens,output_tokens=result.output_tokens);total_provider_cost+=result.cost if result.cost and result.cost>Decimal("0") else calculated_cost
